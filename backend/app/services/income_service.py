...
from app.repositories import income_repo, tag_repo
...
def create_income(
    ...
    return income_repo.create_income(
        db,
        user_id,
        category_id,
        amount,
...
def update_income(
    ...
    return income_repo.update_income(
        db,
        income,
...
def set_archived(db: Session, user_id: int, income_id: int, archived: bool) -> Income:
    income = get_income_or_404(db, user_id, income_id)
    return income_repo.set_archived(db, income, archived)
